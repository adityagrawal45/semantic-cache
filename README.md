...
5. On a miss, the request is routed to the correct provider (`gpt-*` → OpenAI, `claude-*` → Anthropic, `ollama/*` → local Ollama) based on the `model` field. Streaming responses are forwarded to the client in real time while being buffered in the background for storage.
...
| Multi-provider | OpenAI, Anthropic, Ollama (local), routed by `model` prefix |
...
│   ├── providers.py             # OpenAI / Anthropic / Ollama abstraction + streaming
